...
export default async function VaultPage() {
  ...
  let failed = false;
  try {
    posts = (await blogsService.list({ limit: 24 }, { server: true })).items;
  } catch {
    failed = true;
  }
  ...
  if (failed) {
    return (
      <div className="relative z-10 flex min-h-[70vh] items-center justify-center bg-surface/[0.95]">
        <p className="text-sm uppercase tracking-wide text-text-muted">
          The Vault is unavailable right now. Please try again shortly.
        </p>
      </div>
    );
  }
  ...
  const [heroArticle, ...moreArticles] = posts;
